...
class Camera {
 protected:
  int width_;
  int height_;
  Eigen::Isometry3d c2w_;  // camera -> world, sometimes called as "pose"
  Eigen::Isometry3d w2c_;

 public:
  Camera();
  virtual ~Camera();
  Camera(int width, int height);
  Camera(int width, int height, const Eigen::Isometry3d& c2w);
  int width() const;
  int height() const;
  const Eigen::Isometry3d& c2w() const;
  const Eigen::Isometry3d& w2c() const;
  void set_size(int width, int height);
  void set_c2w(const Eigen::Isometry3d& c2w);
...
class PinholeCamera : public Camera {
 ...
 public:
  ...
  PinholeCamera(int width, int height, const Eigen::Isometry3d& c2w);
  PinholeCamera(int width, int height, const Eigen::Isometry3d& c2w,
                float fov_y_deg);
  PinholeCamera(int width, int height, const Eigen::Isometry3d& c2w,
...
class OrthoCamera : public Camera {
 public:
  ...
  OrthoCamera(int width, int height, const Eigen::Isometry3d& c2w);
