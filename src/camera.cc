...
Camera::Camera()
    : width_(-1),
      height_(-1),
      c2w_(Eigen::Isometry3d()),
      w2c_(Eigen::Isometry3d()) {}
...
Camera::Camera(int width, int height, const Eigen::Isometry3d& c2w)
...
const Eigen::Isometry3d& Camera::c2w() const { return c2w_; }

const Eigen::Isometry3d& Camera::w2c() const { return w2c_; }

void Camera::set_size(int width, int height) {
  width_ = width;
  height_ = height;
}
void Camera::set_c2w(const Eigen::Isometry3d& c2w) {
  c2w_ = c2w;
  w2c_ = c2w_.inverse();
}
...
PinholeCamera::PinholeCamera(int width, int height,
                             const Eigen::Isometry3d& c2w)
    : Camera(width, height, c2w),
      principal_point_(-1, -1),
      focal_length_(-1, -1) {}

PinholeCamera::PinholeCamera(int width, int height,
                             const Eigen::Isometry3d& c2w, float fov_y_deg)
    : Camera(width, height, c2w) {
  principal_point_[0] = width_ * 0.5f;
  principal_point_[1] = height_ * 0.5f;
...
OrthoCamera::OrthoCamera(int width, int height, const Eigen::Isometry3d& c2w)
    : Camera(width, height, c2w) {}
