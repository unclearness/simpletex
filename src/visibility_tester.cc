  ...
  for (int k = 0; k < 3; k++) {
    ...
    (*color)[k] =
        (1.0f - local_u) * (1.0f - local_v) *
            image.at(tex_pos_min[0], tex_pos_min[1], k) +
        local_u * (1.0f - local_v) *
            image.at(tex_pos_max[0], tex_pos_min[1], k) +
        (1.0f - local_u) * local_v *
            image.at(tex_pos_min[0], tex_pos_max[1], k) +
        local_u * local_v * image.at(tex_pos_max[0], tex_pos_max[1], k);
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  for (int i = 0; i < vertex_num; i++) {
    ...
    if (option_.use_mask && keyframe_->mask.at(nn_x, nn_y, 0) != 255) {
      continue;
    }
    // nearest depth pixel should be valid (larger than 0)
    if (option_.use_depth && keyframe_->depth.at(nn_x, nn_y, 0) <
    ...
    if (option_.interp == ColorInterpolation::kNn) {
      vertex_info.color[0] = keyframe_->color.at(nn_x, nn_y, 0);
